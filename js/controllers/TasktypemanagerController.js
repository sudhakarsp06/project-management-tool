...
angular.module('UserManagment.controllers').
 controller('TasktypemanagerController', ['$scope','$location','$routeParams','$cacheFactory','$filter','$translate','TasktypeServices','Validationservice','Storageservices','$route','GeneralServices',function( $scope, $location, $routeParams,$cacheFactory, $filter, $translate, TasktypeServices,Validationservice,Storageservices,$route,GeneralServices) {
	...
	$scope.init = function() {	
		...
		$scope.fields = (temp_id)?btfieldManager('edit_tasktype'):btfieldManager('create_tasktype');	
		$scope.fields.id = {value:temp_id};		
		
	}
	
	$scope.gettasktypes = function() {
		TasktypeServices.getTaskTypes(function(response) {
	...
	$scope.getatasktype = function() {
		if( $scope.fields.id.value > 0 ) {
			TasktypeServices.getATaskType({id:$scope.fields.id.value},
				function(data){
					if( data.error ) {
	...
	$scope.addtaskType = function() {	
		...
		if( Validationservice.validate() > 0 ) { //If any error
			...
			TasktypeServices.Add($scope.fields).
			success(function(data){
				if( data.error ) {
					...
					$location.path('/'+$scope.pages.LIST_TASKTYPE+'/1');
				}
			}).error(function(data){
			});
		}
	...
	{		
		...
		TasktypeServices.getTaskTypes($scope.spaginate, function(response) {	  
			...
			$scope.spaginate.pageurl = $scope.pages.LIST_TASKTYPE;		
		}, function() {
	...
	$scope.removeTask = function(id) {
		TasktypeServices.removeTaskType({id:id}, function(response) {
			if(response.error) {
				$scope.response.error = response.error;
			} else {
				$scope.reloadPage();
	...
	$scope.reloadPage = function() {
		...
		$location.path('/'+$scope.pages.LIST_TASKTYPE+'/1').search(urls);
	}
