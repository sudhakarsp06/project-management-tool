...
Flight::route('/getprojects', function() {
	...
	if( !isAuth() ) {		
		...
		$query = $db->prepare("  SELECT p.id, p.title, u.username, p.created_date FROM projects p, users u $sql  $where  LIMIT $limit,$items_per_page");	
...
Flight::route('/gettasksbystatus', function() {
	global $db,$input;
	if( !isAuth() ) {		
		$response['error'] = "You are not authorised to see that page.";		
	} else {
		...
		$query = $db->prepare("  SELECT s.color, s.name, COUNT( t.status_id ) as cnt FROM  `tasks` t, STATUS s WHERE project_id = $id AND s.id = t.status_id GROUP BY t.status_id");	
		$query->execute();
		if( $query->rowCount() ) {
	    while($result = $query->fetch()) {
			//print_r( $result );
